...
그리고 아래의 경우엔 와이어 프레임을 따라 프론트엔드 입장에서 작성해본 유저 플로우이다.
...
위의 내용말고도 짧은 시간에 기능 구현을 마치느라 신경쓰지 못한 아쉬운 코드들이 많다. 반복되는 구조, 패턴에 대해 신경쓰기가 쉽지 않았고 이를 재활용성 높게 구조화하는 것도 크게 신경쓰지 못했다. 해서 이제 앞으로의 시간 동안은 다음과 같은 것들을 하려고 한다.
...
이번 프로젝트의 가장 큰 목적은 `기회자, 디자이너와의 협업`과 실제 운영할 수 있는 작은 서비스였다. 코드 자체에 대한 부분은 아쉬움이 많지만 내가 목표로 했던 것들은 전부다 얻은 거 같아서 만족스럽다. 이후에 진행하는 것들에 대해서는 그 다음 글로 작성해보려 한다.

(아! 끝으로 현재 배포중인 서비스 링크를 남겨두고 간다!)

[베스트 이벤트](https://quarter-fe.vercel.app/)
